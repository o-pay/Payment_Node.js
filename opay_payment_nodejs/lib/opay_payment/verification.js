...
class PaymentVerifyBase{
    constructor(){
        this.param_xml_file = fs.readFileSync(__dirname + '/../../lib/opay_payment/OPayPayment.xml').toString();
    ...
    get_opt_param_pattern(apiname){
        ...
        if (apiname === 'AioCheckOut'){
            pattern['ChoosePayment'].splice(2, 5);
        }
        // console.log(pattern);
        return pattern;
    }
        ...
        Object.keys(params).forEach(function (p_name) {
           ...
           switch (p_type) {
               ...
               case 'DepOpt':
                   ...
                   if (parent_val === 'ALL'){
                    aval_dopt = [];
                    let all_paytype_list = ['ATM', 'Tenpay', 'Credit', 'TopUpUsed', 'WebATM', 'CVS'];
                    all_paytype_list.forEach(function (name){
                      aval_dopt.push(all_dep_opt[name]);
                    });
                    aval_dopt = JSON.stringify(aval_dopt);
                    aval_dopt = aval_dopt.replace(/\[\[\"/g,'"');
                    aval_dopt = aval_dopt.replace(/\]\,\[/g,',');
                    aval_dopt = aval_dopt.replace(/\"\]\]/g,'"');
                    aval_dopt = JSON.parse("[" + aval_dopt + "]");
                    aval_dopt = Array.from(new Set(aval_dopt));
                   }
...
class AioCheckOutParamVerify extends PaymentVerifyBase{
    ...
    verify_aio_payment_param(params){
        if (params.constructor === Object){
            ...
            // if (params === {}){
            //     throw new OPayError.OPayInvalidParam(`Parameter object is empty.`);
            // }
               ...
               if (related_required_param !== undefined && related_required_param !== []){
                   related_required_param.forEach(function (e) {
                       if (!Object.keys(params).includes(e)){
                           throw new OPayError.OPayInvalidParam(`Lack required parameter [${e}] when [${pa}] is set to [${val}].`);
                       }
                   });
               }
            });
    ...
    verify_aio_inv_param(params){
        if (params.constructor === Object){
            ...
            inv_param.forEach(function (pname) {
                if (input_param.indexOf(pname, 0) === -1){
                    throw new OPayError.OPayInvalidParam(`Lack required invoice param ${pname}`);
                }
            });
            input_param.forEach(function (pname) {
                if (inv_param.indexOf(pname, 0) === -1){
                    throw new OPayError.OPayInvalidParam(`Unexpected parameter in Invoice parameters ${pname}`);
                }
            });
            ...
            // if (param_diff !== []){
            //     throw new OPayError.OPayInvalidParam(`Lack required invoice param ${param_diff}`);
            // }
            // let unexp_param = Object.keys(params) - inv_param_names;
            // if (unexp_param !== []){
            //     throw new OPayError.OPayInvalidParam(`Unexpected parameter in Invoice parameters ${unexp_param}`);
            // }
            ...
            if (params['CarruerType'] === '1'){
                ...
                if (params['CustomerID'] !== ''){
                    throw new OPayError.OPayInvoiceRuleViolate(`[CarruerType] can not be empty when [CustomerID] is not empty.`);
                }
            }
            ...
            if (params['TaxType'] === '2'){
                ...
                if (!params['InvoiceItemTaxType'].includes('2')){
                    throw new OPayError.OPayInvoiceRuleViolate(`[InvoiceItemTaxType] must contain at lease one '2' when [TaxType] is 2.`);
                }
            }
            ...
            if (params['CustomerIdentifier'] !== ''){
                if (params['CarruerType'] === '1' || params['CarruerType'] === '2'){
                    throw new OPayError.OPayInvoiceRuleViolate(`[CarruerType] Cannot be 1 or 2 when [CustomerIdentifier] is given.`);
                }
            }
            ...
            if (params['InvoiceItemName'] === ''){
                throw new OPayError.OPayInvoiceRuleViolate(`[InvoiceItemName] is empty.`);
            } else {
                if (params['InvoiceItemName'].includes('|')){
                    ...
                    item_params.forEach(function (param_name) {
                        ...
                        if (item_cnt !== p_cnt){
                            throw new OPayError.OPayInvoiceRuleViolate(`Count of item info [${param_name}] (${p_cnt}) not match count from [InvoiceItemName] (${item_cnt}).`);
                        }
                    });
                    ...
                    if (vio_tax_t === []){
                        throw new OPayError.OPayInvoiceRuleViolate(`Illegal [InvoiceItemTaxType]: ${vio_tax_t}`);
                    }
                    if (params['TaxType'] === '9'){
                        if (!item_tax.includes('1')){
                            throw new OPayError.OPayInvoiceRuleViolate(`[InvoiceItemTaxType] must contain at lease one '1'.`);
                        }
                        if (item_tax.includes('2') && item_tax.includes('3')){
                            throw new OPayError.OPayInvoiceRuleViolate(`[InvoiceItemTaxType] cannot contain 2 and 3 at the same time.`);
                        }
                    }
                } else {
                    // 沒有管線 => 逐一檢查後4項有無管線
                    item_params.forEach(function (param_name) {
                        if (params[param_name].includes('|')){
                            throw new OPayError.OPayInvoiceRuleViolate(`Item info [${param_name}] contain pipeline delimiter but there's only one item in param [InvoiceItemName].`);
                        }
                    });
                }
            }
